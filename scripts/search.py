...
def main():
    ...
    page_results = fetch_shuffled_page(
        rng, args.page, ITEMS_PER_PAGE, args.type,
        genres, platforms, min_year, watched_ids, include_watched,
        language=language, region=region, rt_min_score=rt_min_score
    )

    # Enrich with trailers, platforms, and format output
    final_output = [build_media_card(item, user_genre_ids=genres, language=language, region=region) for item in page_results]
