...
def get_page_shufflers(args_type, genres, platforms, min_year, language='en-US', region='US'):
    """
    ...
    """
    movie_pages = get_tmdb_total_pages('movie', genres, platforms, min_year, language, region) if args_type in ('movie', 'both') else 0
    tv_pages = get_tmdb_total_pages('tv', genres, platforms, min_year, language, region) if args_type in ('tv', 'both') else 0
...
def fetch_shuffled_page(rng, requested_page, items_per_page, media_type_pref, genres, platforms, min_year, watched_ids, include_watched, language='en-US', region='US', rt_min_score=70):
    """
    ...
    """
    movie_page_list, tv_page_list = get_page_shufflers(media_type_pref, genres, platforms, min_year, language, region)
    ...
    while len(valid_results) < target_count and attempts < MAX_ATTEMPTS and (movie_page_list or tv_page_list):
        ...
        if media_type_pref in ('movie', 'both') and movie_page_list:
            ...
            movies = search_tmdb('movie', genres, platforms, current_movie_page, min_year, language, region)
            for m in movies: m['media_type'] = 'movie'

        shows = []
        if media_type_pref in ('tv', 'both') and tv_page_list:
            current_tv_page = tv_page_list.pop(0)
            shows = search_tmdb('tv', genres, platforms, current_tv_page, min_year, language, region)
