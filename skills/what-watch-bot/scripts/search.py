...
def main():
    ...
    logger.info(f"--- STARTING SEARCH PIPELINE --- | Type: '{args.type}' | Seed: {seed} | Legacy Page: {args.page}")
    ...
    try:
        ...
        logger.info(f"Loaded DB Context - Watched items: {len(watched_ids)}, Include watched: {prefs.get('include_watched', False)}")
        logger.info(f"Search Preferences - Genres: {prefs.get('genres', [])}, Platforms: {prefs.get('platforms', [])}")
        ...
        logger.info(f"Triggering pagination fetch for {items_per_page} items...")
        ...
        logger.info(f"--- SEARCH COMPLETE --- | Emitting {len(final_output)} JSON objects to stdout.")

        print(json.dumps(final_output, ensure_ascii=False, indent=2))
        
    except Exception as e:
        logger.error(f"Search pipeline failed with exception: {e}")
        # Print an empty JSON array to stdout so OpenClaw parser doesn't crash
        print("[]")
